...
const TeamTicket = (props, this_ref) => {
	...
	const getMembers = useCallback(() => {
		...
		for(let i = 1; i <= 4; i++) {
			...
			if(member)
				members.push({ image:member.image, name:member.name });
		}
						...
						>
							<div className="flex flex-row h-full justify-center items-center">
								<div className="h-full max-w-[275px] mr-[30px]">
...

									{
										getMembers().map((member,index) => {
											return (
												<div key={index} className="flex flex-row items-center mb-[14px]">
													<div className="w-[40px] h-[40px] flex justify-center items-center flex-shrink-0 rounded-full bg-black mr-[8px] overflow-hidden">
														{ 
															...
															<div className="text-white text-xl font-bold">{member.name[0]}</div>
															:
															<img src={member.image} alt={member.name} className="object-fill h-full w-full"/>
														}
													</div>
								...
								<div className="flex flex-col justify-center items-center mr-[40px]">
									<img width={190} src="/assets/mini_hackathon_logo.webp" alt="Mini hackathon logo" className="mb-[8px]"/>
									<div className="ticket-team-round mb-[28px]">1st Round</div>
									<div className="ticket-team-date mb-[6px]">Sunday, July 24, 2022</div>
									<div className="ticket-team-time mb-[36px]">7PM IST Online</div>
									<div className="w-[183px] h-[40px] flex flex-row items-center">
										<div className="team-hosted-by whitespace-nowrap mr-[8px]">Hosted by</div><img src="/assets/ms_club_logo.png" alt="Mini hackathon logo" className="h-[40px] object-contain"></img>
									</div>
								</div>
							</div>
						</div>
