...
function AwarenessSession() {
    ...
    const [ticket, setTicket] = useState({ display: false, it_no: "", number: 0, name: "", link: "", onRender: null });
    ...
    const onSubmit = async (member_data) => {
        try{
            ...
            setTicket({ ...member_data, onRender, number: String(member_data.number).padStart(4, '0') });
        }catch(error){
    ...
    return (
        <>
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="w-[22em] md:w-[35em] rounded-[5px] p-[2em] md:p-[4em] relative border-2 border-gray-400 overflow-hidden">
                        ...
                        <div className="w-full flex items-center justify-center">
                            <button type="submit" disabled={ status.state === "success" } className="mt-2 w-48 h-10 rounded bg-black text-white hover:bg-gray-300 hover:text-black transition duration-0 hover:duration-500">
                                Register
                            </button>
                        </div>
                    </div>
            ...
            <TicketPopup ticketNo={ticket.number} studentItNo={ticket.it_no} studentName={ticket.name} display={ticket.display} onRender={ticket.onRender} onClose={closePopup}/>
        </>
...
function TicketPopup({ ticketNo, studentItNo, studentName, display, onRender, onClose }) {
    return (
        <div className={`fixed w-screen h-screen top-0 left-0 bg-[#000000d9] flex flex-col items-center backdrop-blur-md ${display ? "" : "z-[-2]"}`}>
            ...
            <button onClick={() => onClose && onClose()} type="submit" className="mt-2 w-24 h-10 rounded bg-black text-white hover:bg-gray-300 hover:text-black transition duration-0 hover:duration-500">
                Close
            </button>
        </div>
    )
